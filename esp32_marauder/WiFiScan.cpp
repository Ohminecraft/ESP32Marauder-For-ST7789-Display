...
void WiFiScan::main(uint32_t currentTime)
{
  ...
  else if (currentScanMode == WIFI_SCAN_ACTIVE_EAPOL)
  {
    ...
    displayString4.concat("AP: ");
    for (int i = 0; i < access_points->size(); i++) {
      if (access_points->get(i).selected) total_ap_selected++;
      if (total_ap_selected <= 2 && total_ap_selected != 0) {
        if (total_ap_selected != 1)
          displayString4.concat(", ");
        displayString4.concat(access_points->get(i).essid);
      }
    }
    ...
    if (currentTime - initTime >= 1000) {
      ...
      for (int x = 0; x < STANDARD_FONT_CHAR_LIMIT; x++)
        ...
        if (total_ap_selected > 2) {
          display_obj.tft.setCursor(18, TFT_HEIGHT / 2.4);
          display_obj.tft.println(", And "  + String(total_ap_selected - 2) + " more...");
        }
        display_obj.showCenterText(displayString2, TFT_HEIGHT / 2);
        display_obj.showCenterText(displayString, TFT_HEIGHT / 2);
      #endif
    ...
    String displayString3 = "";
    String displayString4 = "";
    String displayString5 = "";
    String displayString6 = "";
    int total_ap_selected = 0;
    int total_sta_mac_selected = 0;
    displayString3.concat("Total AP Selected: ");
    displayString4.concat("Total Station Mac Selected: ");
    displayString5.concat("AP: ");
    displayString6.concat("STA: ");
    for (int x = 0; x < access_points->size(); x++) {
      ...
      if (access_points->get(x).selected) {
        ...
        if (total_ap_selected <= 2 && total_ap_selected != 0) {
          if (total_ap_selected != 1)
            displayString5.concat(", ");
          displayString5.concat(access_points->get(x).essid);
        }
        ...
        for (int i = 0; i < cur_ap.stations->size(); i++) {
          ...
          if (stations->get(cur_ap.stations->get(i)).selected) {
            ...
            if (total_sta_mac_selected <= 1 && total_sta_mac_selected != 0) {
              displayString6.concat(macStr);
            }
    ...
    if (currentTime - initTime >= 1000) {
      ...
      for (int x = 0; x < STANDARD_FONT_CHAR_LIMIT; x++)
        displayString2.concat(" ");
      #ifdef HAS_SCREEN
        display_obj.tft.setCursor(18, TFT_HEIGHT / 4);
        display_obj.tft.setTextColor(TFT_GREEN, TFT_BLACK);
        display_obj.tft.println(displayString3);
        display_obj.tft.setCursor(18, TFT_HEIGHT / 3.4);
        display_obj.tft.println(displayString4);
        display_obj.tft.setCursor(18, TFT_HEIGHT / 3);
        display_obj.tft.println(displayString5);
        if (total_ap_selected > 2) {
          display_obj.tft.setCursor(18, TFT_HEIGHT / 2.8);
          display_obj.tft.println(", And "  + String(total_ap_selected - 2) + " more...");
        }
        display_obj.tft.setCursor(18, TFT_HEIGHT / 2.6);
        display_obj.tft.println(displayString6);
        if (total_sta_mac_selected > 1) {
          display_obj.tft.setCursor(18, TFT_HEIGHT / 2.4);
          display_obj.tft.println(", And "  + String(total_sta_mac_selected - 1) + " more...");
        }
        display_obj.showCenterText(displayString2, TFT_HEIGHT / 2);
        display_obj.showCenterText(displayString, TFT_HEIGHT / 2);
      #endif
    ...
    for (int i = 0; i < access_points->size(); i++) {
      if (access_points->get(i).selected) {
        this->broadcastCustomBeacon(currentTime, ssid{access_points->get(i).essid, random(1, 12), {random(256), 
                                                                                                   ...
                                                                                                   random(256)}});
        total_ap_selected++;
        if (total_ap_selected <= 3 && total_ap_selected != 0) {
          if (total_ap_selected != 1)
            displayString4.concat(", ");
          displayString4.concat(access_points->get(i).essid);
    ...
    if (currentTime - initTime >= 1000)
    {
      ...
      for (int x = 0; x < STANDARD_FONT_CHAR_LIMIT; x++)
        ...
        display_obj.tft.println(displayString4);
        if (total_ap_selected > 2) {
          display_obj.tft.setCursor(18, TFT_HEIGHT / 2.2);
          display_obj.tft.println(", And "  + String(total_ap_selected - 2) + " more...");
        }
        display_obj.showCenterText(displayString2, TFT_HEIGHT / 2);
        display_obj.showCenterText(displayString, TFT_HEIGHT / 2);
  ...
  else if ((currentScanMode == WIFI_ATTACK_BEACON_SPAM))
  {
    ...
    if (currentTime - initTime >= 1000)
    {
      ...
      for (int x = 0; x < STANDARD_FONT_CHAR_LIMIT; x++)
        ...
        if (total_list_ap > 2) {
          display_obj.tft.setCursor(18, TFT_HEIGHT / 2.2);
          display_obj.tft.println(", And "  + String(total_list_ap - 2) + " more...");
        }
        display_obj.showCenterText(displayString2, TFT_HEIGHT / 2);
        display_obj.showCenterText(displayString, TFT_HEIGHT / 2);
      #endif
    ...
    for (int i = 0; i < access_points->size(); i++) {
      if (access_points->get(i).selected) {
        ...
        if (total_ap_selected <= 3 && total_ap_selected != 0) {
          if (total_ap_selected != 1)
            displayString2.concat(", ");
          displayString2.concat(access_points->get(i).essid);
        }
      }
    }
    ...
    if (currentTime - initTime >= 1000) {
      ...
      for (int x = 0; x < STANDARD_FONT_CHAR_LIMIT; x++)
        ...
        display_obj.tft.setCursor(18, TFT_HEIGHT / 2.6);
        display_obj.tft.println(displayString2);
        if (total_ap_selected > 2) {
          display_obj.tft.setCursor(18, TFT_HEIGHT / 2.2);
          display_obj.tft.println(", And "  + String(total_ap_selected - 2) + " more...");
        }
